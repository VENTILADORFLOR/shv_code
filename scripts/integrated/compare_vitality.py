...
print(f"合并行数: {len(df)}, {df['date'].min().date()} ~ {df['date'].max().date()}")
...
print("\n各阶段指数均值 (2017=100):")
print(f"  {'阶段':<22} {'GL(Svc)':>10} {'SSEZ(Ind)':>10} {'SHV(Macro)':>11}")
...
print("\n818 冲击（Index_2017 基准）:")
for name, v in shocks.items():
    if v["has_trough"]:
        print(f"  {name}: 最低点 {v['drop_pct']:.1f}% @ {v['trough_date'].strftime('%Y-%m')}"
                                                                         )
    else:
        print(f"  {name}: Monotonic Expansion (+{v['avg_chg_pct']:.1f}% avg)"
                                                                        )

print("\n2025 制裁变化（截至 2026-03，早期信号）:")
...
LABELS = {
        :   "Golden Lions (Service / Informal)",
          : "SSEZ Boundary (Industrial)",
         :  "Sihanoukville Province (Macro)",
}
